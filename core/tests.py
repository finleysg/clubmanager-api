...
class MemberTestCase(TestCase):
    ...
    def setUp(self):
        ...
        Member.objects.create(user=good_user, ghin="123", stripe_customer_id="cus_XYZj3rqj93f")
        Member.objects.create(user=missing_user, ghin="456")
        Member.objects.create(user=bad_user, ghin="654", stripe_customer_id="not-real")
    ...
    def test_good_user_has_stripe_id(self):
        member = Member.objects.get(ghin="123")
        self.assertTrue(member.has_stripe_id())

    def test_missing_user_has_stripe_id(self):
        member = Member.objects.get(ghin="456")
        self.assertFalse(member.has_stripe_id())

    def test_bad_user_has_stripe_id(self):
        member = Member.objects.get(ghin="654")
        self.assertFalse(member.has_stripe_id())
